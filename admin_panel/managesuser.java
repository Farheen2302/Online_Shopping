...
public class managesuser extends HttpServlet
{
...
out.println("<h1>LAPTON</h1> <br> <br> <br> </header> <!--HTML for the Drop Down Menus associated with Top Menu Bar--> <!--They should be inserted OUTSIDE any element other than the BODY tag itself--> <!--A good location would be the end of the page (right above '</BODY>')--> <!--Top Drop Down Menu 1 HTML--> <topper> <pre> <p>  				<br>				  <a href='adminhome.html' >ADMIN HOME</a>     <a href='managebrands' >MANAGE BRANDS</a>         <a href='manageusers' >MANAGE USERS</a>     <a href='manageproduct' >MANAGE PRODUCTS</a>     <a href='manageorders' >MANAGE ORDERS</a>    <a href='logsout' class='myButton'>LOGOUT</a> </P>	 </pre> </topper> <div> <nav> <ul> 		  <li><a href='manageorders'>Manage Orders</a></li> <li><a href='manageproduct'>Manage Products</a></li> <li><a href='managesuser'>Manage Users</a></li> </ul> </nav> </div>  <table>     <thead>         <tr>             <th>USER ID</th>             <th>USER NAME</th>             <th>PHONE NO:</th>             <th>ADDRESS</th>             <th>DELETE</th>         </tr>     </thead>     <tbody>"); 		
	ResultSet rs=stmt.executeQuery("select User_Id,User_Name,ADDRESS,PHONE_NO from REGISTERED_USER"); 	
	while(rs.next()) 		
	{ 			 
     String id=rs.getString(1);
     String name=rs.getString(2);
     long no=Long.parseLong(rs.getString(4));
     String add=rs.getString(3);	 
	out.println("<tr>             <td>"+id+"</td>             <td>"+name+"</td>              <td>"+no+"</td>             <td>"+add+"</td>                       <td> <form method='post' action='deleteuser'>     <input type='hidden' name='userid' value="+id+">             <input type='submit' value='delete'> </form></td>              </tr>");
	}

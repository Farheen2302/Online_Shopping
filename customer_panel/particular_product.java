...
public class particular_product extends HttpServlet
{
...
//<a href='#' id='ex3' title='getAttribute() example' onclick='test3(0)">Link 1</a> - <a href="#" title="Link 2 - setAttribute()" onclick="test3(1)">Link 2</a>
	 ...
	 else
	 {
        ...
        String product_name,product_price,model,brrand,acc,gc,size,ram,color,pro,qty,offer,hd;
        r.next();
        product_name=r.getString(2);
        product_price=r.getString(9);		
        model=r.getString(3);		
	  ...
	  // out.print("<form action='#'>");
	   out.print("<p>           <img src='"+link1+"' height='250px' width='250px'><img src='"+link2+"' height='250px' width='250px'><img src='"+link3+"' height='250px' width='250px'>");
        ...
        out.print(" <font size='5'> <h1 style='color:lightgreen'>"+product_name+"</h1>");
       out.print("   <p>PRICE: $"+product_price+"<br>");
       out.print("MODEL: "+model+"<br>");
       out.print("BRAND: "+brrand+"<br>");
       out.print("SIZE: "+size+"<br>");
       out.print("RAM: "+ram+"<br>");
       out.print("PROCESSOR: "+pro+"<br>");
      // out.print("GRAPHICS CARD: "+gc+"<br>");
       out.print("HD: "+hd+"<br>");
      // out.print("COLOR: "+color+"<br>");
       out.print("QUANTITY: "+qty+"<br></p></font>");
	   
	   if(s!=null)
	   {
	         out.print("<form action='add_to_cart'>");
			 out.print("<label>Quantity:<input type='number' name='quantity' required></label>");
	   		  out.print("<input type='hidden' name='pid' value='"+product_id+"'>"); 
      ...
      out.print("<pre>                              <section class='b9'> <div>                                          <h1 style='color:lightgreen'>COMMENT</h1>");
	  
	  
	  
	  //r=st.executeQuery("select registered_user.User_Name,comment.detail from registered_user join comment on registered_user.User_ID=comment.User_ID where comment.Product_ID="+product_id+"");
	  
	  
	  r=st.executeQuery("select User_ID,Detail from comment where Product_ID='"+product_id+"'");
	  ResultSet rs2;
	  ...
	  if(r.next())
	  {
	    do
		{
		   if(r.getString(1).equals("ADMIN"))
		   ID="ADMIN";
		   else
		   {
		       rs2=st2.executeQuery("select User_Name from registered_user where User_ID='"+r.getString(1)+"'");
			   if(rs2.next())
			   ID=rs2.getString(1);
			   else
			   ID="USER";
		   }
          out.print("<label>"+ID+" : </label>"+r.getString(2)+"");	
        out.print("<br>");		  
	    }while(r.next());
	  }
     else
     out.print("<h3><font color='red'>There are no comments on this product!</font></h3>");


    out.print("</section> </pre> ");
	if(s!=null)
	out.print("<form class='col-1-2' action='comment' > <br> <br>    	<legend>COMMENT</legend>   <br>     <label>       Comment:       <textarea name='comments'></textarea>     </label><br><br><br><br>  <input type='hidden' name='pid' value='"+product_id+"'>  	<button type='submit' name='submit'><strong>       SUBMIT      </strong> </button> <br> <br> <br> </form>");
	out.print("</div> <br>     <footer> Copyright © farheenfnilofer@gmail.com <br> mario <br> bushra <br> <br> </footer>  </body> </html> 	 ");

...
out.print("<pre>                  <h1>                    YOUR TRANSACTION HISTORY..</h1></pre>");
...
//<a href='#' id='ex3' title='getAttribute() example' onclick='test3(0)">Link 1</a> - <a href="#" title="Link 2 - setAttribute()" onclick="test3(1)">Link 2</a>
	 ...
	 else
	 {
		...
		r=st.executeQuery("select * from trans_history where User_ID='"+usrid+"'");
		//out.print("query1");
 String oid,pid,status,date,qty,mode,price;
 long amount;
 if(r.next())
 {
 
 
 out.print(" <table>     <thead>         <tr>             <th>  ORDER ID/th>             <th>PRODUCT ID</th>             <th>ORDER DATE</th>             <th>ORDER AMOUNT</th>                         <th>QUANTITY</th>             <th>ORDER STATUS</th>             <th>MODE</th>                      </tr>     </thead>     <tbody>");
    ...
    do
	{
	 ...
	 // out.print("pid-"+pid);
	  ...
	  rs=st2.executeQuery("select price from product where product_id='"+pid+"'");
	//  out.print("query2");
	  rs.next();
  ...
  out.print("<tr>");
          out.print("  <td>"+oid+"</td>");
           ...
           //  out.print("<form action='del_trans'><td> <p><button type='submit' name='submit' value='"+pid+"'>DELETE</button></a</td></form>");
        out.print("</tr>");
	}while(r.next());
	
	 out.print(" </tbody> </table>    <br> <br>        ");
	 
	 //out.print("<center><font color='red' ><h3>To clear the transaction history</h3></font><form action='del_trans'><button type='submit' name='submit' value='all'>CLEAR</button></form></center>");
	 
  }
  else
  {
    out.print("<font color='red'>No Transaction history!</font>");
   }

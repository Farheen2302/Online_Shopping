	 ...
	 else
    {
	  ...
	  String off=null;
		do
		{
		  ...
		  if(key2.equalsIgnoreCase("offers"))
		  {
		    rr2=st3.executeQuery("select offer_detail from offers where product_id='"+pid+"'");
			rr2.next();
			if(rr2.getString(1)!=null)
			off=rr2.getString(1);
			else
			off=null;
			}
         ...
         out.print("   <p>Rs"+price+"</p>");
		 if(off!=null)
		 {
		    out.print("   <p><font color='red'>offer details : "+off+"</font></p>");
			}
        // out.print("   <p><a href='#'><button type='button'>BUY NOW</button></a><a href='#'><button type='button'>ADD TO WISHLIST</button></a></p>");
         out.print("</a> ");
		if(s!=null)
		{
		 out.print("<label>Quantity:<input type='number' name='quantity'></label>");
		  out.print("<input type='hidden' name='pid_hidden' value='"+pid+"'>"); 
		 
         out.print("   <p></a><a href='b'><button type='submit'>ADD TO WISHLIST</button></p>");
         }
		// out.print("</a>");
		 if(s!=null)
		 out.print("</form>");
